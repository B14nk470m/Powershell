...
$Directorypath = Read-Host -Prompt 'Input the path under which all the user folders exist, use the full path e.g C:\tsg\test' #local base path under which all the user folders exist, such as "F:\UserProfiles", where user folders are "F:\UserProfiles\Username"
...
#######Take Ownership of the root directory
 ...
 ###################### Setup & apply correct folder permissions to the root user folder
  ...
  $DirACL.AddAccessRule((new-object System.Security.AccessControl.FileSystemAccessRule("$env:userdomain\Domain Admins",$fullrights, $inherit, $propagation, "Allow")))
  $DirACL.AddAccessRule((new-object System.Security.AccessControl.FileSystemAccessRule("$env:userdomain\TSGadmin",$fullrights, $inherit, $propagation, "Allow")))
  #System: Full Control
  ...
  $DirACL.AddAccessRule((new-object System.Security.AccessControl.FileSystemAccessRule("$env:userdomain\$Locked.Fullname",$fullrights, $inherit, $propagation, "Allow")))
